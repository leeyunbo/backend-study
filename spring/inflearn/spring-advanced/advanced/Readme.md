...
1. 해당 쓰레드만 접근할 수 있는 특별한 저장소를 의미한다. 
2. 쓰레드 로컬을 사용하면 각 쓰레드마다 별도의 내부 저장소를 제공한다. 
3. 따라서 같은 인스턴스의 쓰레드 로컬 필드에 접근해도 문제가 없다.
4. 쓰레드 로컬을 모두 사용하고 나면 `ThreadLocal.remove()`를 호출하여 저장소를 초기화 해야만 한다.